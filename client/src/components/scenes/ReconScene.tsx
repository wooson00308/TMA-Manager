...
import type { ReconData } from '@shared/schema';

export function ReconScene() {
  const { setScene, enemyTeams } = useGameStore();
  const [selectedEnemyTeam, setSelectedEnemyTeam] = useState(13); // Steel Ravens

  const { data: reconData, isLoading, error } = useQuery<ReconData>({
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {availableTeams.map((team) => (
            <button
              ...
              className={`cyber-border p-3 text-sm transition-all ${
                selectedEnemyTeam === team.id
                  ? 'bg-pink-900/30 border-pink-400/50 text-pink-400'
                  : 'bg-slate-800 border-slate-600 text-gray-300 hover:border-pink-400/30'
              }`}
            >
              {team.name}
            </button>
